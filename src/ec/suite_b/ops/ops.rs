...
pub struct PublicScalarOps {
    ...
    // XXX: `PublicScalarOps` is designed to avoid having the ECDSA
    // verification code depend on `PrivateKeyOps`. But, it needs to depend on
    // `PrivateKeyOps` for now until the implementation of `twin_mul` is
    // improved.
